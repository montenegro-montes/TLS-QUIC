...
for nombre_archivo in os.listdir(directorio_entrada):
    ...
    with open(ruta_completa, mode='r', newline='', encoding='utf-8') as f:
        ...
        for fila in lector:
            ...
            try:
                longitud = int(fila["Length"])
            except ValueError:
                continue

            if protocolo == "TCP":
                suma_tcp += longitud
            elif protocolo == "TLSv1.3":
                suma_tls += longitud

    suma_total = suma_tcp + suma_tls
    agrupado_por_firma[sig_alg].append([kem_alg, suma_tcp, suma_tls, suma_total])
...
for sig_alg, filas in agrupado_por_firma.items():
    ...
    with open(nombre_salida, mode='w', newline='', encoding='utf-8') as f:
        ...
        escritor.writerow(['KEM_ALG', 'Suma_TCP', 'Suma_TLS', 'Suma_Total'])
